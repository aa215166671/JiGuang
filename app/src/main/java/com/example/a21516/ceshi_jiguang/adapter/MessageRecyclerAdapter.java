...
import com.example.a21516.ceshi_jiguang.view.CircleImageView;
...
public class MessageRecyclerAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder>{
    ...
    private void addList(List<MessageBean> newList){
        ...
        if (holder instanceof ItemViewHolder){
            ...
            if (!data.equals("")){
                ...
                if (data.get(position).type==0){
                    //把String转化为“1”
                   // ((ItemViewHolder) holder).infotime.setVisibility(View.VISIBLE);
                    if (Integer.valueOf(data.get(position).time)<=0){
                       ...
                       // ((ItemViewHolder) holder).infotime.setText(TimeUtils.unix2Date("MM-dd HH:mm",));
                    }else{
                        ((ItemViewHolder) holder).time.setText(data.get(position).time+"条未读消息");
                        ((ItemViewHolder) holder).time.setTextColor(Color.parseColor("#E5955D"));
                       // ((ItemViewHolder) holder).infotime.setText(TimeUtils.ms2date("MM-dd HH:mm",));
                    }
    ...
    class ItemViewHolder extends RecyclerView.ViewHolder{
        ...
        TextView title,time,content ,infotime;
        CircleImageView img;
        Button button;
        public ItemViewHolder(View view) {
            ...
            infotime = (TextView)view.findViewById(R.id.item_main_infotime);
        }
    }
}
